...
def build_data(triples):
    ...
    for s,r,t,gs,gt,tid in triples:
        ...
        p=canon_pair(gs,gt); all_pairs.add(p); pair_rows[p].append((s,t,r,tid))
...
def score_identity_pair_local(adj0,adj1,aliases,p,method):
    a,b=p; s1=local_score(adj1,a,b,method)
...
def svd_embeddings(adj,rank,seed):
    ...
    X=svd.fit_transform(A); C=svd.components_
    return idx,X,C
...
def score_identity_pair_svd(idx0,X0,C0,idx1,X1,C1,aliases,p):
    a,b=p; s1=svd_pair_score(idx1,X1,C1,a,b)
...
def stable_tie_topk(keys,scores,k,rep):
    scores=np.asarray(scores); k=min(k,len(keys)); tie=[]
    for key in keys:
        tie.append(hashlib.sha256(f'tie|{LOCK_SEED}|{rep}|{key[0]}|{key[1]}'.encode()).hexdigest())
    order=sorted(range(len(keys)),key=lambda i:(-float(scores[i]),tie[i]))[:k]
    return {keys[i] for i in order}


def tie_robust_ridi(keys,s0,s1,k):
    vals=[ridi(stable_tie_topk(keys,s0,k,rep),stable_tie_topk(keys,s1,k,rep)) for rep in range(TIE_REPS)]
    q=np.quantile(vals,[0.025,0.5,0.975]); return {'tie_ridi_q025':float(q[0]),'tie_ridi_median':float(q[1]),'tie_ridi_q975':float(q[2])}


def bootstrap_ridi(keys,s0,s1,k):
    n=len(keys); rng=np.random.default_rng(LOCK_SEED+k); vals=[]; s0=np.asarray(s0); s1=np.asarray(s1)
    for _ in range(BOOT_REPS):
        ix=rng.integers(0,n,n); kk=min(k,n)
        o0=ix[np.argsort(-s0[ix],kind='mergesort')[:kk]]; o1=ix[np.argsort(-s1[ix],kind='mergesort')[:kk]]
        vals.append(ridi({keys[i] for i in o0},{keys[i] for i in o1}))
    q=np.quantile(vals,[0.025,0.5,0.975]); return {'boot_ridi_q025':float(q[0]),'boot_ridi_median':float(q[1]),'boot_ridi_q975':float(q[2])}
...
def eval_method(name,keys,labels,s0,s1):
    ...
    for k in K_VALUES:
        ...
        t0=topk_set(keys,s0,k); t1=topk_set(keys,s1,k)
        ...
        x.update(tie_robust_bounds(keys,s0,s1,k)); x.update(tie_robust_ridi(keys,s0,s1,k)); x.update(bootstrap_ridi(keys,s0,s1,k)); rows.append(x)
    return rows


def negative_control(adj0,aliases,keys):
    old=sorted(adj0); perm=sorted(old,key=lambda x:hashlib.sha256(f'perm|{LOCK_SEED}|{x}'.encode()).hexdigest()); mp={o:f'P{j:08d}' for j,o in enumerate(perm)}
    adp=defaultdict(set)
...
def main():
    ...
    triples,bad=load_jsonl(paths); aliases,pair_rows,train_pairs,test_pairs,all_pairs=build_data(triples)
    entities=sorted({x for p in train_pairs for x in p}); neg=sample_negatives(entities,all_pairs,min(len(test_pairs)*NEGATIVE_RATIO,100000))
    keys=sorted(test_pairs)+sorted(neg); labels=np.array([1]*len(test_pairs)+[0]*len(neg),dtype=np.int8); adj0,adj1=build_graphs(pair_rows,train_pairs)
    metrics=[]
    for method in ('jaccard','aa','ra'):
        s0=[]; s1=[]
        for p in keys:
    ...
    negctl=negative_control(adj0,aliases,keys); pd.DataFrame(metrics).to_csv(out/'openkg_ridi_metrics.csv',index=False); pd.DataFrame(negctl).to_csv(out/'negative_control.csv',index=False)
