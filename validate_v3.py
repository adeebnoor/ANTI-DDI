...
check(state_counts.get("ANTI_DDI_SUPPORTED", 0) == 538,
      "538 records are evidence-supported Anti-DDI states")
check(state_counts.get("ANTI_DDI_CANDIDATE_LIMITED", 0) == 106,
      "106 records are limited Anti-DDI candidates")
check(state_counts.get("STRUCTURAL_CONTROL_ONLY", 0) == 44,
      "44 records are structural controls")
check(state_counts.get("UNRESOLVED", 0) == 76,
      "76 records are unresolved")
check(state_counts.get("POSITIVE_CONCERN_EXCLUDED", 0) == 33,
      "33 records are excluded for positive concern")

check(len(bench) == 538, "default benchmark has 538 rows")
check(set(bench["knowledge_state"]) == {"ANTI_DDI_SUPPORTED"},
      "default benchmark contains only ANTI_DDI_SUPPORTED records")
...
check(len(split) == 214, "Paper 5 manifest has 214 pairs")
check(split_counts.get("development", 0) == 40, "Paper 5 development split has 40 pairs")
check(split_counts.get("confirmatory", 0) == 174, "Paper 5 confirmatory split has 174 pairs")
check(set(split["pair_id"]).issubset(set(v3["pair_id"])),
      "all Paper 5 pairs exist in the v3 dataset")

check(len(anchors) == 8, "clinical anchor table has 8 pairs")
check(set(anchors["pair_id"]).issubset(set(bench["pair_id"])),
      "all clinical anchors belong to the default T1/T2 benchmark")
...
check((anchors["analysis_status"] == "post-confirmatory supportive").all(),
      "clinical anchors are explicitly marked post-confirmatory/supportive")

check(not (v3.loc[v3["knowledge_state"] == "UNRESOLVED", "recommended_use"] == "DEFAULT_BENCHMARK").any(),
      "UNRESOLVED records are never default negatives")
check(not (v3.loc[v3["knowledge_state"] == "POSITIVE_CONCERN_EXCLUDED", "recommended_use"] == "DEFAULT_BENCHMARK").any(),
      "positive-concern exclusions are never default negatives")
